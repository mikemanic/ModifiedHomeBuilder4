...
# CORRECTED IMPORT: assemblies is a subfolder inside the current folder
...
class hb_sample_cabinets_OT_cabinet_prompts(bpy.types.Operator):
    ...
    def update_drawer_boxes(self, context):
        for carcass in self.cabinet.carcasses:
            if not carcass.exterior:
                continue
            
            exterior = carcass.exterior
            
            # GET MATERIAL THICKNESS
            # Default to 18mm if prompt not found
            mat_thick = pc_unit.millimeter(18)
            try:
                mat_thick_prompt = carcass.get_prompt("Material Thickness")
                if mat_thick_prompt:
                    mat_thick = mat_thick_prompt.distance_value
            except:
                pass

            # CALCULATE GLOBAL DIMENSIONS FOR THE BOXES
            # Width: Design Carcass width - 2 * material thickness
            box_width = carcass.obj_x.location.x - (2 * mat_thick)
            
            # Depth: Design Carcass depth - 20mm
            box_depth = math.fabs(carcass.obj_y.location.y) - pc_unit.millimeter(20)
            
            if box_depth < 0.01: # Prevent negative depth
                box_depth = 0.01

            # IDENTIFY DRAWER FRONTS AND BOXES
            # We assume Drawer Fronts are children of the Exterior BP and do NOT have the IS_DRAWER_BOX_BP tag
            drawer_fronts = []
            existing_boxes = []
            
            for child in exterior.obj_bp.children:
                if 'IS_DRAWER_BOX_BP' in child:
                    existing_boxes.append(pc_types.Assembly(child))
                else:
                    # Heuristic: Assume other children are fronts or assemblies containing fronts
                    # You might need to check for 'IS_DRAWER_FRONT_BP' if your system uses it
                    drawer_fronts.append(pc_types.Assembly(child))

            # MATCH BOXES TO FRONTS OR CREATE NEW ONES
            # We try to match based on Z location (Height position)
            used_boxes = []

            for front in drawer_fronts:
                front_height = math.fabs(front.obj_z.location.z)
                
                # DETERMINE BOX HEIGHT (Z)
                # Logic based on Front Height (in Meters)
                if 0.08 <= front_height <= 0.12:
                    box_height = 0.07
                elif 0.12 < front_height <= 0.18:
                    box_height = 0.10
                elif 0.18 < front_height <= 0.28:
                    box_height = 0.15
                elif 0.28 < front_height <= 0.35:
                    box_height = 0.20
                else:
                    # Fallback for sizes outside range, or smaller than 80
                    box_height = 0.07

                # FIND AN EXISTING BOX AT THIS HEIGHT
                matched_box = None
                for box in existing_boxes:
                    if box in used_boxes:
                        continue
                    
                    # Check if vertical centers align roughly (within 1cm)
                    box_z_loc = box.obj_bp.location.z + (box.obj_z.location.z / 2)
                    front_z_loc = front.obj_bp.location.z + (front.obj_z.location.z / 2)
                    
                    if math.fabs(box_z_loc - front_z_loc) < 0.01:
                        matched_box = box
                        used_boxes.append(box)
                        break
                
                # CREATE NEW BOX IF NO MATCH FOUND
                if not matched_box:
                    try:
                        # Call the function from your other file
                        matched_box = assembly_cabinet.add_drawer_box(exterior)
                        used_boxes.append(matched_box)
                    except Exception as e:
                        print("Error adding drawer box: " + str(e))
                        continue

                if matched_box:
                    # UPDATE DIMENSIONS
                    matched_box.obj_x.location.x = box_width
                    matched_box.obj_y.location.y = -box_depth
                    matched_box.obj_z.location.z = box_height

                    # UPDATE LOCATION TO MATCH FRONT
                    matched_box.obj_bp.location.x = front.obj_bp.location.x
                    matched_box.obj_bp.location.z = front.obj_bp.location.z
                    matched_box.obj_bp.location.y = front.obj_bp.location.y # Usually 0 relative to exterior

                    # UPDATE OR CREATE PROMPTS
                    try:
                        # Prompt for OBJ_Y
                        p_y = matched_box.get_prompt("OBJ_Y")
                        if not p_y:
                            p_y = matched_box.add_prompt("OBJ_Y", 'DISTANCE', box_depth)
                        else:
                            p_y.distance_value = box_depth
                            
                        # Prompt for OBJ_Z
                        p_z = matched_box.get_prompt("OBJ_Z")
                        if not p_z:
                            p_z = matched_box.add_prompt("OBJ_Z", 'DISTANCE', box_height)
                        else:
                            p_z.distance_value = box_height
                    except:
                        pass
