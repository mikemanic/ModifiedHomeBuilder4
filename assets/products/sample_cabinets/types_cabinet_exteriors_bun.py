...
class Doors(types_fronts.Fronts):
    ...
    def update_prompts_after_placement(self,context):
        ...
        if width > pc_unit.millimeter(447):
...
class Drawers(types_fronts.Fronts):
    ...
    drawer_qty = 5              # Number of drawers faces on the cabinet
    ...
    def draw(self):     
        ...
        self.add_prompt("Drawer Quantity",'QUANTITY',4)
    ...
    def update_prompts_after_placement(self,context):
        ...
        qty_prompt.set_value(min(math.ceil(height/pc_unit.millimeter(200)),8))
...
class Door_Drawer(types_fronts.Fronts):
    ...
    def draw(self):
        ...
        l_drawer_front.loc_y('IF(inset,front_thickness,-door_to_cabinet_gap)-(y*open_drawer/100)',[inset,front_thickness,door_to_cabinet_gap,y,open_drawer])
        ...
        # ==========================================
        # --- FIXED: LEFT DRAWER BOX (Split Logic) ---
        # ==========================================
        
        l_box = assemblies_cabinet.add_drawer_box(self)
        l_box.set_name('DrawerBoxLeft')
        l_box.rot_x(value = 0)
        l_box.rot_y(value = 0)
        l_box.rot_z(value = 0)
        
        # Dimensions
        # Logic: If split, width is half minus gap. If single, width is full.
        l_box.dim_x('IF(add_two_drawer_fronts_var,(x-front_thickness)/2,x)',[add_two_drawer_fronts_var,x,front_thickness])
        l_box.dim_y('y+.5', [y]) 
        l_box.dim_z('top_df_height_var-0.05', [top_df_height_var])
        
        # Positioning
        # X: Left aligned (0) works for both Single and Left-of-Split.
        l_box.loc_x(value = 0)
        l_box.loc_y('-(y*(open_drawer/100))',[y, open_drawer])
        l_box.loc_z('z+front_thickness-0.003-top_df_height_var', [z, to_var, top_df_height_var, front_thickness])
        
        # Hide logic: NEVER hide this box. It acts as the single drawer box when not split,
        # and the left box when split.
        ...
        r_drawer_front.loc_y('IF(inset,front_thickness,-door_to_cabinet_gap)+(y*open_drawer/100)',[inset,front_thickness,door_to_cabinet_gap,y,open_drawer])
        ...
        # ==========================================
        # --- FIXED: RIGHT DRAWER BOX (Split Logic) ---
        # ==========================================
        
        r_box = assemblies_cabinet.add_drawer_box(self)
        r_box.set_name('DrawerBoxRight')
        r_box.rot_x(value = 0)
        r_box.rot_y(value = 0)
        r_box.rot_z(value = 0)
        
        # Dimensions
        r_box.dim_x('(x-front_thickness)/2',[x, front_thickness])
        r_box.dim_y('y+.5',[y])
        r_box.dim_z('top_df_height_var-0.05', [top_df_height_var])
        
        # Positioning
        r_box.loc_x('x/2+front_thickness/2',[x, front_thickness])
        r_box.loc_y('-(y*(open_drawer/100))',[y, open_drawer])
        r_box.loc_z('z+front_thickness-vertical_gap-top_df_height_var', [z, to_var, top_df_height_var, front_thickness, vertical_gap])
        
        # Hide if not two drawers
